...
@interface ZWSpellListType02View()<UITableViewDelegate,UITableViewDataSource>
...
@implementation ZWSpellListType02View
...
- (void)createTableViewWithCell:(UITableViewCell *)cell withIndex:(NSInteger)index {
    ...
    if (index == 0) {
        ...
        detailLabel.text = [NSString stringWithFormat:@"%@m²",self.model.size];
    }else if (index == 5) {
        detailLabel.text = [self valueText:self.model.hallNumber];
    }else if (index == 6) {
        detailLabel.text = [NSString stringWithFormat:@"%@",self.model.exhibitorCount];
    }else if (index == 7) {
