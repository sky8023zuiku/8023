...
- (void)rightItemClick:(UIBarButtonItem *)item {
    ...
    [[ZWShareManager shareManager]showShareAlertWithViewController:self withDataModel:model withExtension:[self takeExtension] withType:1];

}

- (NSDictionary *)takeExtension {
    NSDictionary *extension = @{
        @"zw_status":@"1",//0为邀请二维码
        @"coverImages":[NSString stringWithFormat:@"%@%@",httpImageUrl,self.shareModel.coverImages],
        @"zw_content":@{
...
- (void)createUI {
    ...
    self.delBtn.frame = CGRectMake(0.05*kScreenWidth , kScreenHeight-zwTabBarStausHeight-0.13*kScreenWidth-zwNavBarHeight, 0.9*kScreenWidth, 0.1*kScreenWidth);
    [self.delBtn setTitle:@"投递名片" forState:UIControlStateNormal];
...
//    [self.navigationController pushViewController:messageVC animated:YES];
    ...
    cardListVC.type = 0;
    cardListVC.merchantId = self.model.merchantId;
...
- (void)createTableViewCell:(UITableViewCell *)cell cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        if (self.contactArray.count == 0) {
            ...
            [phoneBtn setImage:[UIImage imageNamed:@"icon_card_phone"] forState:UIControlStateNormal];
...
- (void)requestExExhibitorsProductList {
    if (self.shareModel.exhibitorId) {
        __weak typeof (self) weakSelf = self;
        [[ZWDataAction sharedAction]postFormDataReqeustWithURL:zwExExhibitorProductList parametes:@{@"exhibitorId":self.shareModel.exhibitorId,@"status":@"2"} successBlock:^(NSDictionary * _Nonnull data) {
            __strong typeof (weakSelf) strongSelf = weakSelf;
            if (zw_issuccess) {
...
- (void)downloadBtnClick:(UIButton *)btn {
    ...
    NSString *urlStr = [NSString stringWithFormat:@"%@%@",httpImageUrl,[array objectAtIndex:1]];
    NSString *fileName = [NSString stringWithFormat:@"%@",[array objectAtIndex:0]]; //获取文件名称
    [[ZWPdfManager shareManager]downloadPdfProfile:self withUrl:urlStr withFileName:fileName];
