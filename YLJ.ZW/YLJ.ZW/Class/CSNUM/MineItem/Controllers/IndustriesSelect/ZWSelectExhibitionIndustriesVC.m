//
//  ZWSelectExhibitionIndustriesVC.m
//  YLJ.ZW
//
...
@interface ZWSelectExhibitionIndustriesVC ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation ZWSelectExhibitionIndustriesVC
...
- (void)right2ItemClcik:(UINavigationItem *)item {
    ...
    if (self.industriesIds.count == 0) {
        [self showOneAlertWithMessage:@"展会行业不能为空，请选择后提交"];
        return;
    }
    __weak typeof (self) weakSelf = self;
...
- (void)submitIndustriesInformation {
    ...
    __weak typeof (self) weakSelf = self;
    [[ZWDataAction sharedAction]postReqeustWithURL:zwUpdateExhibitionIndustriesList parametes:parametes successBlock:^(NSDictionary * _Nonnull data) {
        __strong typeof (weakSelf) strongSelf = weakSelf;
        if (zw_issuccess) {
            [[ZWAlertAction sharedAction]showOneAlertTitle:@"提示" message:@"展会行业变更成功" confirmTitle:@"我知道了" actionOne:^(UIAlertAction * _Nonnull actionOne) {
                __strong typeof (weakSelf) strongSelf = weakSelf;
                [[NSNotificationCenter defaultCenter]postNotificationName:@"refreshCanReadExhibitionIndustryList" object:nil];
                [strongSelf.navigationController popViewControllerAnimated:YES];
            } showInView:strongSelf];
        }else {
            [strongSelf showOneAlertWithMessage:@"展会行业变更失败，请稍后再试或联系客服"];
        }
