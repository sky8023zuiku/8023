...
- (instancetype)initWithFrame:(CGRect)frame andDelegate:(id)delegate andTitlArr:(NSArray *)titleArr {
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        [self prepareLayout:0];
    }
    
    return self;
}

- (void)prepareLayout:(NSInteger )index {
    
    self.dfSegmentHead = [[DFSegmentHeadView alloc] initWithFrame:CGRectZero];
    self.dfSegmentHead.selectIndex = index;
...
-(void)setSelectIndex:(CGFloat)selectIndex {
    ...
    [self prepareLayout:selectIndex];
}
- (void)reloadData {
...
//    [self prepareLayout:self.selectIndex];
...
- (void)selectWithIndex:(NSInteger)index {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(didSelectSegmentView:selectTitleWithIndex:)]) {
        
        [self.delegate didSelectSegmentView:self selectTitleWithIndex:index];
    }
