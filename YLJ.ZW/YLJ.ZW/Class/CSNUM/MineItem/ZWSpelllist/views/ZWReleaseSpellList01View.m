...
@interface ZWReleaseSpellList01View()<UITableViewDelegate, UITableViewDataSource, YYTextViewDelegate, ZWAreaManagerDelegate, CSDateManagerDelegate>
...
@implementation ZWReleaseSpellList01View
...
- (instancetype)initWithFrame:(CGRect)frame withModel:(ZWSpellListModel *)model withType:(NSInteger)type;
{
    ...
    if (self) {
        self.model = model;
        self.model.exhibitorSize = self.model.exhibitorCount;
        self.type = type;
...
- (void)setSelectType:(NSString *)selectType {
    if (self.model.type.length == 0) {
        ...
        if ([selectType isEqualToString:@"1"]) {
            mydic = [[ZWMineSaveSpellListAction shareAction]takeOneSpellList];
        }else if ([selectType isEqualToString:@"2"]) {
            mydic = [[ZWMineSaveSpellListAction shareAction]takeTwoSpellList];
        }else {
            mydic = [[ZWMineSaveSpellListAction shareAction]takeThreeSpellList];
        }
...
//                self.model = [ZWSpellListModel parseJSON:mydic];
                ...
                self.model.exhibitorCount = self.model.exhibitorSize;
            }
        }else {
            self.model.exhibitorSize = self.model.exhibitorCount;
        }
...
- (void)createTableViewCell:(UITableViewCell *)cell cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        NSArray *titleArray = @[@"标题：",@"展会名称：",@"展馆名称：",@"布展时间：",@"展期：",@"截止时间：",@"城市：",@"展台面积：",@"展厅：",@"材质：",@"展台数量："];
        ...
        if (indexPath.row == 0) {
            ...
            UITapGestureRecognizer *tapOne = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(tapDateClick:)];
            [self.decorateStratData addGestureRecognizer:tapOne];
            
            UILabel *aLabel = [[UILabel alloc]initWithFrame:CGRectMake(CGRectGetMaxX(self.decorateStratData.frame), 0, 0.06*kScreenWidth, 0.1*kScreenWidth)];
            aLabel.text = @"至";
            ...
            self.decorateEndData = [[UITextField alloc]initWithFrame:CGRectMake(CGRectGetMaxX(aLabel.frame), 0.02*kScreenWidth, 0.3*kScreenWidth, 0.06*kScreenWidth)];
            ...
            self.decorateEndData.rightView = rightView02;
            ...
            UITapGestureRecognizer *tapTwo = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(tapDateClick:)];
            [self.decorateEndData addGestureRecognizer:tapTwo];
            ...
            self.exhibitionStartTime.tag = 1003;
            ...
            self.exhibitionEndTime.tag = 1004;
            ...
            self.sepllInvalidDate.tag = 1005;
            ...
            self.cityText.tag = indexPath.row;
            [self.cityText addTarget:self action:@selector(valueText:) forControlEvents:UIControlEventAllEditingEvents];
            ...
            UITextField *valueText = [[UITextField alloc]initWithFrame:CGRectMake(0.205*kScreenWidth, 0, 0.65*kScreenWidth, 0.1*kScreenWidth)];
            valueText.placeholder = @"请输入展厅";
            valueText.font = smallMediumFont;
            valueText.tag = indexPath.row;
            valueText.text = self.model.hallNumber;
            [valueText addTarget:self action:@selector(valueText:) forControlEvents:UIControlEventAllEditingEvents];
            [cell.contentView addSubview:valueText];
        }else if (indexPath.row == 9) {
            UITextField *valueText = [[UITextField alloc]initWithFrame:CGRectMake(0.205*kScreenWidth, 0, 0.65*kScreenWidth, 0.1*kScreenWidth)];
            valueText.placeholder = @"请输入材质";
            valueText.font = smallMediumFont;
            valueText.tag = indexPath.row;
            valueText.text = self.model.material;
            [valueText addTarget:self action:@selector(valueText:) forControlEvents:UIControlEventAllEditingEvents];
            [cell.contentView addSubview:valueText];
        }else {
            UITextField *valueText = [[UITextField alloc]initWithFrame:CGRectMake(0.205*kScreenWidth, 0, 0.65*kScreenWidth, 0.1*kScreenWidth)];
            valueText.placeholder = @"请输入展台数量";
            valueText.font = smallMediumFont;
            valueText.tag = indexPath.row;
            valueText.keyboardType = UIKeyboardTypeNumberPad;
            valueText.text = self.model.exhibitorCount;
            [valueText addTarget:self action:@selector(valueText:) forControlEvents:UIControlEventAllEditingEvents];
            [cell.contentView addSubview:valueText];
        }
...
- (void)valueText:(UITextField *)textField {
    if (textField.tag == 0) {
        ...
        self.model.hallNumber = textField.text;
    }else if (textField.tag == 9) {
        self.model.material = textField.text;
    }else if (textField.tag == 10) {
        self.model.exhibitorSize = textField.text;
...
- (void)submitBtnClick:(UIButton *)btn {
    ...
    if (self.model.hallNumber.length == 0) {
        [self showAlertWithMessage:@"展厅不能为空"];
        return;
    }
    if (self.model.exhibitorSize.length == 0) {
         [self showAlertWithMessage:@"展台数量不能为空"];
         return;
    }
...
- (void)removeLocationSepllList {
    if ([self.model.type isEqualToString:@"1"]) {
        [[ZWMineSaveSpellListAction shareAction]removeOneSpellList];
    }else if ([self.model.type isEqualToString:@"2"]) {
        [[ZWMineSaveSpellListAction shareAction]removeTwoSpellList];
    }else {
        [[ZWMineSaveSpellListAction shareAction]removeThreeSpellList];
    }
}
...
- (void)saveBtnClick:(UIButton *)btn {
    ...
    NSDictionary *mydic =  [[ZWToolActon shareAction]dicFromObject:self.model];
    if ([self.model.type isEqualToString:@"1"]) {
        [[ZWMineSaveSpellListAction shareAction]saveOneSpellList:mydic];
    }else if ([self.model.type isEqualToString:@"2"]) {
        [[ZWMineSaveSpellListAction shareAction]saveTwoSpellList:mydic];
    }else {
        [[ZWMineSaveSpellListAction shareAction]saveThreeSpellList:mydic];
    }
