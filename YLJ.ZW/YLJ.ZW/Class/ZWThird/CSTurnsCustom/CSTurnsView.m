...
- (void)initialize{
    ...
    subviewClassName = @"CSBaseTurnsView";
    /*
    ...
    switch (self.orientation) {
        case CSTurnsViewOrientationHorizontal:{
            ...
            for (NSInteger i = self.visibleRange.location; i < self.visibleRange.location + _visibleRange.length; i++) {
                CSBaseTurnsView *cell = [_cells objectAtIndex:i];
...
- (void)reloadData
{
    ...
    if (_needsReload) {
        ...
        switch (self.orientation) {
            case CSTurnsViewOrientationHorizontal://横向
...
- (UIView *)dequeueReusableCell{
    CSBaseTurnsView *cell = [_reusableCells lastObject];
    if (cell)
    {
