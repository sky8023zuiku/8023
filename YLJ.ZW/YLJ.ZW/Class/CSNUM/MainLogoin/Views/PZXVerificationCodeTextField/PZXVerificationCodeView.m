...
-(void)setView{
    ...
    for (int i = 0 ; i<self.VerificationCodeNum; i++) {
        ...
        tf.font = [UIFont fontWithName:@"PingFang-SC-Medium" size:20];
...
//        [tf addTarget:self action:@selector(textFieldChanged:) forControlEvents:UIControlEventEditingChanged];
        ...
        UIView *line =[[UIView alloc]initWithFrame:CGRectMake((tf.ff_width - 45)/2, self.frame.size.height-2, 45, 2)];
        line.backgroundColor = [UIColor ff_colorWithHexString:@"33B4FB"];
        [tf addSubview:line];
