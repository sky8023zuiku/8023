...
#define PickerHeight 0.65*screenWith


@interface CSTimePickerView()<UIPickerViewDataSource, UIPickerViewDelegate>
{
    NSInteger yearRange;
    NSInteger dayRange;
    NSInteger startYear;
...
@implementation CSTimePickerView
...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        UIView *contentV = [[UIView alloc] initWithFrame:CGRectMake(0, screenHeight, screenWith, PickerHeight)];
        contentV.backgroundColor = [UIColor whiteColor];
        [self addSubview:contentV];
        self.contentV = contentV;
        
        self.pickerView = [[UIPickerView alloc]initWithFrame:CGRectMake(0, 40, [UIScreen mainScreen].bounds.size.width, PickerHeight-40)];
        ...
        upVeiw.backgroundColor = skinColor;
        ...
        UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width/2-50, 0, 100, 40)];
        label.text = @"选择日期";
...
- (void)showDateTimePickerView{
    ...
    [UIView animateWithDuration:0.25f animations:^{
        self.alpha = 1;
        _contentV.frame = CGRectMake(0, screenHeight-PickerHeight, screenWith, PickerHeight);
...
- (void)hideDateTimePickerView{
    ...
    [UIView animateWithDuration:0.2f animations:^{
        self.alpha = 0;
        _contentV.frame = CGRectMake(0, screenHeight, screenWith, PickerHeight);
    } completion:^(BOOL finished) {
...
//    }
//    if (self.delegate != nil && [self.delegate respondsToSelector:@selector(createDatePickerView:withDateStr:)]) {
//        [self.delegate createDatePickerView:self withDateStr:_string];
//    }
    
    if ([CSDateManager shareManager].delegate != nil && [[CSDateManager shareManager].delegate respondsToSelector:@selector(takeDateWithStr:withIndex:)]) {
        [[CSDateManager shareManager].delegate takeDateWithStr:_string withIndex:self.tag];
    }
    [self hideDateTimePickerView];
}
