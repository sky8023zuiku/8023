...
@interface ZWSelectIndustriesVC_v2 ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation ZWSelectIndustriesVC_v2
...
- (void)takeIndustriesModel:(NSNotification *)notice {
    ...
    if (self.industriesArray.count <= 5) {
        ...
        if (self.industriesArray.count<5) {
            ...
            [self showOneAlertWithMessage:@"每个展商只能选择五个行业"];
...
- (void)createNavigationBar {
    ...
    if (self.type == 1) {
        ...
        [[YNavigationBar sharedInstance]createRightBarWithTitle:@"确认" barItem:self.navigationItem target:self action:@selector(right1ItemClcik:)];
