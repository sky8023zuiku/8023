...
@interface ZWExhibitorsVC ()<ZWTopSelectViewDelegate,UITableViewDelegate,UITableViewDataSource,ZWIndustryMerchantCellDelegate, CSFilterManagerDelegate>
...
- (void)showMenu:(UIBarButtonItem *)item {
    [[CSFilterManager shareManager]showExhibitorsFilterMenu:self setSelectedData:self.selectData];
...
- (void)takeFilterData:(NSDictionary *)data {
    ...
    self.industriesId = data[@"industry"][@"id"];
    self.countries = data[@"country"][@"name"];
    self.cities = data[@"city"][@"name"];
    self.leftListId = @"";
    self.selectType = 0;
    [self createRequest:1];
}
...
- (void)takeMyLevel3Industries {
    ...
    for (NSDictionary *myDic in myArray) {
        ZWExhibitorIndustryModel *model = [ZWExhibitorIndustryModel mj_objectWithKeyValues:myDic];
        [myIndArray addObject:model];
    }
