...
//    }];
    __weak typeof(self) weakSelf = self;
    [[ZWDataAction sharedAction]getReqeustWithURL:zwExhibitionServerLBT parametes:@{@"type":@"2"} successBlock:^(NSDictionary * _Nonnull data) {
        __strong typeof (self) strongSelf = weakSelf;
        if (zw_issuccess) {
            strongSelf.lbtAarry = data[@"data"];
            [strongSelf.tableView reloadData];
        }
...
- (void)createNavigationBar {
    ...
    if (cityDic) {
        ...
        [self.LeftItem setTitle:@"全部" forState:UIControlStateNormal];
    }
