...
@interface ZWReleaseSpellList02View()<UITableViewDelegate, UITableViewDataSource, CSDateManagerDelegate, YYTextViewDelegate, ZWAreaManagerDelegate>
...
@implementation ZWReleaseSpellList02View
...
- (void)setSelectType:(NSString *)selectType {
    if (self.model.type.length == 0) {
        ...
        NSDictionary *mydic = [[ZWMineSaveSpellListAction shareAction]takeFourSpellList];
...
- (void)createTableViewCell:(UITableViewCell *)cell cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        NSArray *titleArray = @[@"标题：",@"展会名称：",@"展馆名称：",@"展会时间：",@"截止时间：",@"城市：",@"展台面积：",@"展厅：",@"展台数量："];
...
- (void)saveBtnClick:(UIButton *)btn {
    ...
    [[ZWMineSaveSpellListAction shareAction]saveFourSpellList:mydic];
...
- (void)removeLocationSepllList {
    [[ZWMineSaveSpellListAction shareAction]removeFourSpellList];
}
