...
- (void)pairBtnClick:(UIButton *)btn {
    ZWFilterConditionsVC *conditionsVC = [[ZWFilterConditionsVC alloc]init];
    conditionsVC.exhibitionId = self.exhibitionId;
    conditionsVC.industries = self.industryArray;
    [self.navigationController pushViewController:conditionsVC animated:YES];
}
...
- (nullable UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
    ...
    if (tableView == _leftTableView) {
        ...
        pairBtn.frame = CGRectMake(0.02*kScreenWidth, 0.025*kScreenWidth, 0.15*kScreenWidth, 0.15*kScreenWidth);
        [pairBtn setBackgroundImage:[UIImage imageNamed:@"pair_icon"] forState:UIControlStateNormal];
        [pairBtn addTarget:self action:@selector(pairBtnClick:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)createRequestIndustry {
    ...
    [[ZWDataAction sharedAction]getReqeustWithURL:zwExhibitionExhibitorIndustryList parametes:parametes successBlock:^(NSDictionary * _Nonnull data) {
        ...
        if (zw_issuccess) {
            ...
            [myArray addObject:model];
            for (NSDictionary *myDic in myData) {
                model = [ZWIndustrysModel mj_objectWithKeyValues:myDic];
                [myArray addObject:model];
            }
